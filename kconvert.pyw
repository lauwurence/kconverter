...
class FolderSettings:
    ...
    def from_dict(cls, data):
        ...
        for preset in settings.presets:
            defaults = default_webm_settings()
            defaults.update(preset.webm)
            preset.webm = defaults
...
class PresetDialog(QDialog):
    ...
    def add_images_settings(self, form):
        self.downscale = QDoubleSpinBox()
        self.downscale.setRange(0.01, 100.0)
        self.downscale.setDecimals(2)
        self.downscale.setSingleStep(0.1)
        self.downscale.setValue(self.preset.downscale)
        form.addRow("Downscale:", self.downscale)
    ...
    def add_webm_settings(self, form):
        ...
        settings.update(self.preset.webm)
    ...
    def accept(self):
        ...
        if self.mode == "Images":
            self.preset.downscale = self.downscale.value()
            ...
            if self.preset.min_quality > self.preset.max_quality:
                QMessageBox.warning(
                    self,
                    "Invalid quality",
                    "Min quality cannot be greater than Max quality.",
                )
                return
        else:
...
class LocalWebMDialog(QDialog):
    def __init__(self, folder, preset, settings, parent=None):
        ...
        self.preset = preset
        self.settings = default_webm_settings()
        self.settings.update(settings)
        self.deleted = False

        self.setWindowTitle(
            f"Local WebM Settings — {preset.name}"
        )
        self.setWindowIcon(QIcon(ICON))
        self.resize(620, 620)
        ...
        info = QLabel(
            f"<small>Folder: {self.folder}<br>"
            f"Preset: {preset.name}<br>"
            "These settings override the global WebM preset "
            "only for this folder.</small>"
        )
        ...
        self.delete_button.setEnabled(
            (self.folder / LOCAL_WEBM_FILE).exists()
        )
    ...
    def delete_local(self):
        result = QMessageBox.question(
            self,
            "Delete local settings",
            f"Delete local WebM settings for '{self.preset.name}' "
            f"in:\n{self.folder}?",
        )
...
class SettingsDialog(QDialog):
    def __init__(self, settings, parent=None):
        ...
        edit_button.clicked.connect(self.edit_preset)
    ...
    def add_preset(self):
        ...
        if (
            PresetDialog(preset, self.settings.mode, self).exec()
...
class ThumbnailWorker(QThread):
    ...
    def cache_file(self, path):
        try:
            stat = path.stat()
            key = (
                f"{path.resolve()}|{stat.st_mtime_ns}|"
                f"{stat.st_size}"
            ).encode()
        except OSError:
...
class ImageConverter:
    ...
    EXIF_DATA = {
        ...
        33432: (
            ...
            "keyclap. All Rights Reserved."
        ),
    }

    def __init__(self, source, preset, stop_event=None):
    ...
    def resolve_output(self):
        ...
        if not output.is_absolute():
            output = self.source.parent / output
        ...
        if found and found[0] and found[0][0] and found[0][1]:
            ...
            for folder in relative_folder.split("/"):
                ...
                if folder == "00_parts":
                    ...
                    converted_name += "_"
                    converted_name += "".join(
                        char for char in folder if not char.isdigit()
                    )

            converted_name = converted_name.replace("__", "_")
            output_name = (
                f"{converted_name}_{file_number:02d}_"
                f"{file_subnumber:03d}{self.suffix}.jpg"
            )
        else:
    ...
    def get_output_file(self, file):
        ...
        relative_folder = file.parent.relative_to(self.source)
    ...
    def scan(self):
        ...
        for folder, dirs, names in os.walk(self.source):
            ...
            for name in names:
                ...
                relative_folder = file.parent.relative_to(self.source)
                output_file = (
                    self.output / relative_folder / output_name
                )
                ...
                if (
                    output_file.exists()
                    and self.cache.get(relative_file) == mod_time
                ):
                    continue
    ...
    def convert_file(self, source, data):
        if self.stop_event.is_set():
            return
        ...
        with Image.open(source) as image:
            image.thumbnail(
                (
                    image.width / self.downscale,
                    image.height / self.downscale,
                ),
                self.RESAMPLE,
            )
            ...
            while True:
                ...
                if (
                    file_size <= self.target_size * 1024
                    or quality <= self.minimum_quality
                ):
                    rgb.save(
                        output,
                        format="JPEG",
                        quality=min(100, round(quality * multiplier)),
                        icc_profile=icc_profile,
                        exif=exif,
                    )
                    break
        ...
        self.source_sizes.append(source_size)
        self.saved_sizes.append(saved_size)
        self.saved_qualities.append(quality)
        self.saved_images += 1
        self.cache[relative_file] = mod_time

        self.log(
            f"{self.saved_images}/{len(self.output_files)}: "
            f"{output} | {rgb.width}x{rgb.height} | "
            f"{quality:.0f}% | {self.human_size(saved_size)}"
        )
...
class WebMConverter:
    ...
    def __init__(
        ...
        self.preset = preset
        self.settings = default_webm_settings()
        self.settings.update(preset.webm)
    ...
    def get_output_file(self):
        return (
            self.output_folder
            / f"{self.folder.name}{self.preset.suffix}.webm"
        )
    ...
    def resolution(self):
        value = self.settings.get("resolution", "Original")
    ...
    def get_source_signature(self, images):
        ...
        data = {
            ...
            "preset": {
                "suffix": self.preset.suffix,
                "output_folder": self.preset.output_folder,
            },
        }
    ...
    def create_concat_file(self, images):
        digest = hashlib.sha1(
            str(self.folder).encode()
        ).hexdigest()

        concat_file = self.folder / f".kconverter_{digest}.txt"

        fps = (
            float(self.settings["input_fps"])
            * float(self.settings["speed"])
        )
        duration = 1.0 / fps
    ...
    def build_filters(self):
        ...
        if effective_fps < output_fps:
            if interpolate == 0:
                ...
                filters.append(
                    MINTERPOLATE[interpolate].format(output_fps)
                )
        else:
        ...
        if sharpen:
            filters.append(
                "unsharp=luma_msize_x=3:luma_msize_y=3:"
    ...
    def run(self):
        ...
        self.log(f"Resolution: {settings['resolution']}")
        self.log(
        ...
        if output.exists():
            ...
            self.log(
                f"Finished: {output} "
                f"({self.format_size(output.stat().st_size)})"
            )
    ...
    def run(self):
        try:
            ...
            for index, job in enumerate(self.jobs, 1):
                ...
                self.message.emit(
                    f"[{index}/{total}] {settings.mode} | "
                    f"{preset.name} | {folder}"
                )
...
class MainWindow(QMainWindow):
    ...
    def update_title(self):
        name = (
            f'{Path(self.project_filename).stem} [{self.project_filename}]'
    ...
    def create_root_controls(self, settings):
        ...
        edit_button.clicked.connect(
            lambda checked=False, s=settings:
    ...
    def add_folder_item(self, settings):
        ...
        item.setSizeHint(
            0,
            QSize(0, ROOT_ROW_HEIGHT),
        )

        self.tree.addTopLevelItem(item)

        # Root items use the same stable path index as nested folders.
        ...
        self.tree.setItemWidget(
            item,
            0,
            self.create_folder_widget(settings, True),
        )
        self.tree.setItemWidget(
            item,
            1,
        ...
        try:
            images = sorted(
                (
                    ...
                    if path.is_file()
                    and path.suffix.lower() in WebMConverter.IMAGE_EXTENSIONS
                ),
    ...
    def create_folder_status(
        ...
        for index, preset in enumerate(settings.presets):
            ...
            if settings.mode == "Images":
                size = self.get_folder_output_size(
                    settings,
                    preset,
                    folder,
                )
            else:
                size = self.get_webm_output_size(
                    settings,
                    preset,
                    folder,
                )
            ...
            button.setToolTip(
                f"Convert only this folder using "
                f"'{preset.name}'"
            )
            button.setFixedHeight(25)
            button.setMinimumWidth(65)

            # A preset without an Output Folder cannot be used for conversion.
            # Keep it visible in the Status column, but make it visibly disabled.
            ...
            if settings.mode == "WebM":
                ...
                if has_local:
                    local_button.setIcon(QIcon("icons/settings_local.svg"))
                else:
                    local_button.setIcon(QIcon("icons/settings.svg"))
            ...
            if settings.mode == "Images":
                ...
                # WebM: размер файла является кнопкой открытия анимации.
        ...
        all_button.setToolTip(
            "Convert this folder using all presets"
        )
        all_button.setFixedHeight(25)
        all_button.setMinimumWidth(50)
        all_button.clicked.connect(
            lambda checked=False,
            s=settings,
            f=Path(folder):
            self.start_folder_all_conversions(s, f)
        )
    ...
    def create_file_status(self, settings, source):
        ...
        for index, preset in enumerate(settings.presets):
            ...
            output, status = self.get_file_status(
                source,
                preset,
            )
    ...
    def get_file_status(self, source, preset):
        ...
        try:
            if (
                ...
                return output, "Outdated"

            return output, self.format_size(
                output.stat().st_size
            )
        except OSError:
    ...
    def edit_local_webm(
        ...
        local_data = self.read_local_webm_settings(
            folder
        )
        current = local_data.get(preset.name)

        base = default_webm_settings()
        base.update(preset.webm)

        if current:
            base.update(current)

        dialog = LocalWebMDialog(
            folder,
            preset,
            base,
            self,
        ...
        self.write_local_webm_settings(
            folder,
            local_data,
        )
    ...
    def start_thumbnail_worker(self):
        ...
        for path in self.items_by_path:
            ...
            if item_path.is_dir():
                # Root folders must never receive thumbnails.
                # Nested folders still use their first image as a thumbnail.
    ...
    def remove_root_folder(self, settings):
        result = QMessageBox.question(
            self,
            "Remove folder",
            "Remove this root folder from the project?"
            f"\n\n{settings.source_folder}",
            QMessageBox.StandardButton.Yes
            | QMessageBox.StandardButton.No,
        )
        ...
        self.rescan()
        self.log_message(
            f"Removed: {settings.source_folder}"
        )

    def get_tree_state(self):
        """Capture view state using stable filesystem paths."""
        expanded = {
            path
    ...
    def restore_tree_state(
        ...
        """Restore folders and defer scroll restoration until layout is ready."""
    ...
    def _restore_scroll_position(
        ...
        self.tree.verticalScrollBar().setValue(
            max(0, int(vertical_scroll))
        )
        self.tree.horizontalScrollBar().setValue(
            max(0, int(horizontal_scroll))
        )

    def rescan(self):
        # Always snapshot the view before destroying QTreeWidgetItems.
        ...
        self.log_message("Folders rescanned.")
    ...
    def start_conversion(
        ...
        if not preset.output_folder:
            QMessageBox.warning(
                self,
                "Output folder",
                f"Preset '{preset.name}' has no "
                "Output folder.",
            )
            return

        local = (
            self.get_local_preset(
                folder,
                preset,
            )
            if settings.mode == "WebM"
    ...
    def start_folder_all_conversions(
        self,
        settings,
        folder,
    ):
    ...
    def read_project(self, filename):
        try:
            ...
            self._tree_state_to_restore = {
                "expanded": set(
                    str(folder_path)
                    for folder_path in ui_state.get("expanded", [])
                ),
                "vertical_scroll": int(
                    ui_state.get("vertical_scroll", 0)
                ),
                "horizontal_scroll": int(
                    ui_state.get("horizontal_scroll", 0)
                ),
            }
            ...
            for folder_data in data.get("folders", []):
                settings = FolderSettings.from_dict(
                    folder_data
                )
